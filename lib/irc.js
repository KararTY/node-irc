    ...
    self.addListener('raw', function(message) {
        ...
        switch (message.command) {
            ...
            case 'rpl_whoreply':
                ...
                break;
            case '690':
                self._addWhoisData(message.args[2], 'lang')
                self._addWhoisData(message.args[2], 'language')
                break;
            case 'rpl_liststart':
                self.channellist = [];
                ...
                break;
            case 'rpl_list':
                if (!self.channellist) self.channellist = [];
                channel = {
                    name: message.args[1],
            ...
            case 'JOIN':
                ...
                self.emit('join', message.args[0], { nick: message.nick, user: message.user }, message);
                self.emit('join' + message.args[0], { nick: message.nick, user: message.user }, message);
                if (message.args[0] != message.args[0].toLowerCase()) {
                    self.emit('join' + message.args[0].toLowerCase(), { nick: message.nick, user: message.user }, message);
                }
                break;
            case 'PART':
                // channel, who, reason
                self.emit('part', message.args[0], { nick: message.nick, user: message.user }, message.args[1], message);
                self.emit('part' + message.args[0], { nick: message.nick, user: message.user }, message.args[1], message);
                if (message.args[0] != message.args[0].toLowerCase()) {
                    self.emit('part' + message.args[0].toLowerCase(), { nick: message.nick, user: message.user }, message.args[1], message);
                }
                if (self.nick == message.nick) {
                    channel = self.chanData(message.args[0]);
            ...
            case 'rpl_youreoper':
                ...
                break;
            case 'rpl_notopic':
                self.emit('notopic', message);
                break;
            case 'rpl_time':
                self.emit('time', message)
            default:
...
Client.prototype.customSend = function(command) {
    if (!this.conn.requestedDisconnect) {
        if (this.opt.debug) console.log(command + '\r\n')
